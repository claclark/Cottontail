...
int main(int argc, char **argv) {
  ...
  bool include_idf = false;
  bool include_unstemmed = false;
  bool include_rsj = true;
  if (argc > 2 &&
      (argv[1] == std::string("-f") || argv[1] == std::string("--full"))) {
    include_idf = true;
    include_unstemmed = true;
    --argc;
    argv++;
  }
  if (argc == 3 || argc == 4) {
  ...
  if (!cottontail::tf_idf_annotations(warren, &error, include_unstemmed,
                                      include_idf, include_rsj)) {
    std::cerr << program_name << ": " << error << "\n";
    return 1;
  }
  warren->end();
