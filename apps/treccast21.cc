...
void usage(std::string program_name) {
  std::cerr << "usage: " << program_name
            << " [--noc4] [--random] [--passages] [2020|2021]\n";
}
...
int main(int argc, char **argv) {
  ...
  std::string cast20_burrow = "/data/hdd3/claclark/cast20.burrow";
  std::string veggie6_cast20_burrow = "/scratchC/claclark/cast20.burrow";
  std::string cast21_burrow = "/data/hdd3/claclark/cast21.burrow";
  std::string veggie6_cast21_burrow = "/scratchC/claclark/cast21.burrow";
  std::string cast_burrow, veggie6_cast_burrow;
  bool c4_feedback = true;
  if (argc > 1 && argv[1] == std::string("--noc4")) {
    c4_feedback = false;
    --argc;
    argv++;
  }
  bool random_results = false;
  if (argc > 1 &&
      (argv[1] == std::string("-r") || argv[1] == std::string("--random"))) {
    random_results = true;
    --argc;
    argv++;
  }
  ...
  if (argc == 1 || (argc == 2 && argv[1] == std::string("2020"))) {
    cast_burrow = cast20_burrow;
    veggie6_cast_burrow = veggie6_cast20_burrow;
  } else if (argc == 2 && argv[1] == std::string("2021")) {
    cast_burrow = cast21_burrow;
    veggie6_cast_burrow = veggie6_cast21_burrow;
  } else {
    usage(program_name);
    return 1;
  }
  ...
  if (c4_feedback) {
    c4_warren = cottontail::Warren::make(simple, c4_burrow, &error);
  ...
  std::string pid = ":pid";
  {
    std::unique_ptr<cottontail::Hopper> pid_hopper =
        cast_warren->hopper_from_gcl(pid, &error);
    ...
    if (random_results) {
      if (labelq.size() == 0)
        return 0;
      size_t depth = 1000 * labelq.size();
      std::string paragraph = ":paragraph";
      std::vector<cottontail::RankingResult> results =
          random_ranking(cast_warren, paragraph, depth);
      size_t n = results.size() / labelq.size();
      if (n == 0)
        return 0;
      size_t i = 0;
      while (!labelq.empty()) {
        std::string label = labelq.front();
        labelq.pop();
        for (size_t rank = 1; rank <= n; rank++) {
          cottontail::addr pid_p, pid_q;
          pid_hopper->tau(results[i].container_p(), &pid_p, &pid_q);
          std::string pid = cottontail::trec_docno(
              cast_warren->txt()->translate(pid_p, pid_q));
          std::cout << label << " Q0 " << pid << " " << rank << " "
                    << results[i].score() << " claclark";
          if (print_passages) {
            std::string passage = clean(cast_warren->txt()->translate(
                pid_q + 1, results[i].container_q()));
            std::cout << "\t" << passage << "\n";
          } else {
            std::cout << "\n";
          }
  ...
  std::string pipeline =
      ...
      "rsj:expansions=25.6182 rsj stem bm25:b=0.481409 bm25:k1=1.02941 bm25";
    ...
    for (;;) {
      ...
      if (c4_feedback) {
        ...
        rankings.emplace_back(
            bm25_ranking(cast_warren, weighted_query, parameters));
...
#if 0
      for (size_t rank = 0; rank < 20 && rank < results.size(); rank++) {
        std::cout << "++++++" << label << " (#" << rank + 1 << "): " << question
                  << "\n";
        std::string contents = cast_warren->txt()->translate(
            results[rank].container_p(), results[rank].container_q());
        std::cout << contents << "\n";
      }
#endif
#if 0
      std::cout << "++++++ " << label << " " << question << "\n";
      for (auto &term : terms)
        if (stopwords.find(term) == stopwords.end())
          std::cout << label << " " << term << " " << weighted_query[term]
                    << "\n";
#endif
      std::flush(std::cout);
      output_mutex.unlock();
